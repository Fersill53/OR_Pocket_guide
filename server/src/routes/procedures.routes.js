...
// POST /api/procedures  (AUTH REQUIRED)
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    console.error('❌ Error creating procedure:', err);
    res.status(400).json({ message: 'Invalid procedure data', error: err.message || String(err) });
  }
});

// PUT /api/procedures/:id (AUTH REQUIRED)
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    console.error('❌ Error updating procedure:', err);
    res.status(400).json({ message: 'Invalid update data', error: err.message || String(err) });
  }
});

// DELETE /api/procedures/:id (AUTH REQUIRED)
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // ✅ IMPORTANT: prevent crashes when id is not a valid Mongo ObjectId
    // e.g. "/api/procedures/add" or "/api/procedures/new"
    if (!isValidObjectId(id)) {
      return res.status(404).json({ message: 'Procedure not found' });
    }

    const proc = await Procedure.findById(id).lean();
